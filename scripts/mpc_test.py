...
from spawn_peds import spawn_peds, reset_peds
...
# main flow-
# args passed
# carla client, world setup
# load lstm model
# prepare vehicle with camera nd everything
# load map
# pedestrians
# mpc
...
def main():
    ...
    client = carla.Client(args.host, args.port) # The main purpose of the client object is to get or change the world, and apply commands
    client.set_timeout(2.0) # seconds. This limits all networking operations so that these don't block the client forever. An error will be returned if connection fails.
    IM_WIDTH = 1280
    IM_HEIGHT = 720
    
    
    world = client.get_world() # A client can connect and retrieve the current world
    traffic_manager = client.get_trafficmanager(args.tm_port) # This module is in charge of every vehicle set to autopilot to recreate urban traffic.
    traffic_manager.set_global_distance_to_leading_vehicle(5.0) # Sets the minimum distance in meters that vehicles have to keep with the rest. The distance is in meters and will affect the minimum moving distance. It is computed from center to center of the vehicle objects.
    if args.hybrid:
        traffic_manager.set_hybrid_physics_mode(True) # In this mode, vehicle's farther than a certain radius from the ego vehicle will have their physics disabled. Computation cost will be reduced by not calculating vehicle dynamics. Vehicles will be teleported.

    print("sync: " + str(args.sync))
    if args.sync:
        CARLA_HZ = 20
        settings = world.get_settings() 
        traffic_manager.set_synchronous_mode(True) # TM is designed to work in synchronous mode. Both the CARLA server and TM should be set to synchronous in order to function properly. 
        settings.synchronous_mode = True
        settings.fixed_delta_seconds = float(1/CARLA_HZ) #0.05 To run the simulation at a fixed time-step of 0.05 seconds apply the following settings. In this case, the simulator will take twenty steps (1/0.05) to recreate one second of the simulated world.
        world.apply_settings(settings)

    print("Settings: " + str(world.get_settings()))

    world.tick() # to synchronize
    ...
    for trial in range(numTrialLow,numTrialLow+numTrial):
        ...
        try:
            ...
            blueprint_library = world.get_blueprint_library() # These layouts allow the user to smoothly incorporate new actors into the simulation. They are already-made models with animations and a series of attributes
            vehicle_bp = blueprint_library.filter('vehicle.toyota.prius*')[0]

            initpoint = carla.Transform(carla.Location(x=-48.81999023, y=-18.79507507, z=0.6), carla.Rotation(roll=0, pitch=0, yaw=89.83876)) # Class that defines a transformation, a combination of location and rotation, without scaling
            ...
            # spawns a camera sensor, attaches it to a vehicle, and tells the camera to save the images generated to disk.
            ...
            wps = wplist[81][1].next_until_lane_end(1.0) # Returns a list of waypoints from this to the end of the lane separated by a certain distance(m)
            for w in wps:
